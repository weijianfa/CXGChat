	...
	if (!msgObj.empty())
	{
		...
		case 1:
		{
				  ...
				  else if (action == "15") // ÷±≤•Õ∆¡˜≥…π¶
				  {
					  ...
					  if (nMasterId == strtoll(m_nMasterId, NULL, 10))
					  {
						  if (m_pObserver)
						  {
		...
		case 2:
		{
				  ...
				  else if (action == "1") // π´ø™µƒÀΩ¡ƒ
				  {
					  strMsg += "∂‘";
					  strMsg += unescape((char*)(*itc)["toMasterNick"].asString().c_str());
					  strMsg += "Àµ£∫";
				  }
				  else if (action == "2") // ÀΩ¡ƒ
				  {
					  strMsg += "∂‘ƒ„Àµ£∫";
				  }
				  else if (action == "3") // µØƒª
				  {
					  return;
				  }
